...
public class IronShulkerBoxes implements ModInitializer {
  ...
  private static void addWaterCauldronInteraction(net.minecraft.world.item.Item item, CauldronInteraction interaction) {
    CauldronInteraction.WATER.map().put(item, interaction);
  }
  ...
  private static final CauldronInteraction SHULKER_BOX = (blockState, level, blockPos, player, interactionHand, itemStack) -> {
    ...
    if (!(block instanceof AbstractIronShulkerBoxBlock shulkerBoxBlock)) {
      ...
      if (!level.isClientSide) {
        ...
        itemstack = itemStack.transmuteCopy(itemstack.getItem(), itemStack.getCount());
  ...
  public void onInitialize() {
    ...
    for (DyeColor color : DyeColor.values()) {
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.IRON_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.GOLD_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.DIAMOND_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.COPPER_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.CRYSTAL_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
      addWaterCauldronInteraction(IronShulkerBoxesBlocks.OBSIDIAN_SHULKER_BOXES.get(color).asItem(), SHULKER_BOX);
